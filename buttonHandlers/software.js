...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const ticketName = `software-${userId}-${Date.now()}`;
            
            const thread = await interaction.channel.threads.create({
                name: ticketName,
                type: ChannelType.PrivateThread,
                reason: `Software ticket created by ${interaction.user.tag}`
            });
            ...
            const ticketMessage = createTicketEmbed('software', `<@${userId}>`, interaction.guild);
